...
const { OAuth2Strategy, OAuthStrategy } = require('passport-oauth');
	...
	new JwtStrategy(
		...
		(jwt_payload, done) => {
			User.findById(jwt_payload._id, (err, user) => {
				...
				if (!user) {
					return done(null, false, {
						msg: `Email ${jwt_payload.email} not found.`,
					});
				}
				return done(null, user);
			});
...
exports.isAuthenticatedJwt = (req, res, next) => {
	...
	try {
		...
		console.error('Error in jwt isAuthenticatedJwt function', e);
		return;
	}
